...
# 定义你的JSON文件名
file_path = '/root/hjh/llminference/originalvllm/vllm-0.5.0.post1/chiahao/train_data/json/data3.jsonl'
...
try:
    ...
    for i, item in enumerate(data):
        ...
        if prompt_len + output_len > 131072: # 切换成对应模型的最大上下文，opt是2048
            ...
            continue
        filtered_dataset.append((index, prompt, prompt_len, output_len, 'arxiv'))
        index += 1
    filtered_dataset_sample = random.sample(filtered_dataset, index)

    data_filename = '/root/hjh/llminference/vllm_inert/vllm-0.5.0.post1/plugin/EfficientServe/sample_tasks_from_datasets/sampled_experiment_datasets/sampled_arxiv-qwen2-5-14b.json'
