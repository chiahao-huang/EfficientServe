...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        # 新增
        shared_cache: torch.Tensor, # @黄佳浩
    ) -> torch.Tensor:
        # --- 核心改进：根据元数据判断是否存在需要使用隐藏缓存的请求 ---
        if attn_metadata.num_hidden_cache_tokens != 0:
            # --- 路径A：批次中存在使用隐藏缓存的请求 ---
            # **存储操作**：对于需要存储隐藏状态的请求，调用自定义算子将hidden_states写入缓存
