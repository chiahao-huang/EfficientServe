...
template <typename scalar_t>
__global__ void adaptiv_dynamic_kv_to_cache_kernel(
...
void adaptive_dynamic_kv_to_cache(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      key.scalar_type(), "adaptive_dynamic_kv_to_cache", [&] {
        adaptiv_dynamic_kv_to_cache_kernel<scalar_t>
            <<<grid, block, 0, stream>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("adaptive_dynamic_kv_to_cache", &adaptive_dynamic_kv_to_cache, "adaptivedynamickv2cache kernel launcher");
}
